...
class VocalRecorder {
  ...
  setupMediaRecorder() {
    ...
    // Lower bitrate for better mobile compatibility and performance
    // 128kbps is sufficient for high-quality vocal recording
    options.audioBitsPerSecond = isMobileDevice() ? 128000 : 192000;

    // Use processed stream for both mobile and desktop to ensure audio capture
    // Raw stream might not work properly on some mobile devices
    ...
    try {
      ...
      console.log('✅ MediaRecorder initialized with mimeType:', this.mimeType, 'bitrate:', options.audioBitsPerSecond, 'stream: processed');
    } catch (error) {
      ...
      try {
        ...
        console.log('✅ MediaRecorder fallback initialized with mimeType:', this.mimeType);
      } catch (fallbackError) {
  ...
  async handleRecordingStop() {
    try {
      ...
      // Try to convert to WAV for better cross-browser compatibility
      // If conversion fails, use original blob
      try {
        this.recordedAudioBuffer = await this.decodeAudioData(blob);
        // Resample to 44100 Hz if needed for better compatibility
        const targetSampleRate = 44100;
        if (this.recordedAudioBuffer.sampleRate !== targetSampleRate) {
          console.log('🔄 Resampling audio from', this.recordedAudioBuffer.sampleRate, 'to', targetSampleRate, 'Hz');
          this.recordedAudioBuffer = await this.resampleAudioBuffer(this.recordedAudioBuffer, targetSampleRate);
        }
        ...
        console.log('✅ WAV conversion successful with sample rate:', targetSampleRate, 'Hz');
      } catch (decodeError) {
        console.warn('WAV conversion failed, using original blob for playback:', decodeError);
        this.recordedAudioBuffer = null;
        this.recordedWavBlob = null;
        this.recordedWavBlobUrl = null;
      }
  ...
  start() {
    ...
    // Small timeslices can cause audio gaps and cutting issues
    // iOS requires no timeslice parameter
    if (isIOS()) {
      this.mediaRecorder.start();
    } else if (isMobileDevice()) {
      this.mediaRecorder.start(1000); // 1000ms for mobile (increased from 100ms)
    } else {
      this.mediaRecorder.start(1000); // 1000ms for desktop (increased from 250ms)
    }
    this.isRecording = true;
    this.recordingStartTimestamp = Date.now();
    this.recordingStartAudioTime = this.audioEngine.getContext().currentTime;
    console.log('🔴 Recording started with timeslice:', isIOS() ? 'none (iOS)' : '1000ms (unified)');
  }
...
class StudioManager {
  ...
  async renderStudioMix(beatVol = 70, vocalVol = 80) {
    ...
    const sampleRate = Math.max(vocalBuffer.sampleRate, beatBuffer.sampleRate, 44100);
    const totalDuration = Math.max(vocalBuffer.duration, beatBuffer.duration);
